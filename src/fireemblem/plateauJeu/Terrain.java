...
import fireemblem.parser.xml.ChapitreParser;
import fireemblem.parser.xml.Parser;
import fireemblem.parser.xml.PlateauParser;
...
public class Terrain extends GameEnvironnementAbstract {
    ...
    public GamePlatformAbstract createGamePlatform () {
    	...
    	switch (chapitres) {
    		case blazing_sword:
    			Parser.Parse("src/fireemblem/xml/map/blazing_sword.xml", new PlateauParser(this, plateauDeJeu));
    			Parser.Parse("src/fireemblem/xml/personnage/blazing_sword.xml", new ChapitreParser(plateauDeJeu));
    			break;
    		default :
    	        for (int i = 0 ; i < 20 ; i++) {
    	            for (int j = 0 ; j < 20; j++) {
