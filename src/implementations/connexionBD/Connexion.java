package implementations.connexionBD;
...
import implementations.strategie.StrategieImmobile;
...
public class Connexion extends Methode {
	...
	private void savePersonnagesPartie (List<CharacterAbstract> personnages, String type) {
        ...
        for (int i = 0 ; i < personnages.size() ; i++) {
        	Personnage perso = (Personnage) personnages.get(i);
        	int id = 0;
        	if ("personnage".equals(type)) {
	...
	private void savePersonnagePartie (Personnage perso, PreparedStatement prepare, PreparedStatement prepareObjet, int id) {
		this.setParameterInt(prepare, this.partie, 1);
	...
	public void loadPersonnagesPartie (String type, PlateauJeu plateauJeu) {
        ...
        while (this.aResult(result)) {
    		Personnage perso = this.loadPersonnagePartie(result);
			...
			while (this.aResult(resultObjet)) {
				Objet objet = factoryObjet.createObjet(this.getStringResultByString(resultObjet, "nom"), TypeObjet.valueOf(this.getStringResultByString(resultObjet, "type_objet")));
				objet.setUtilisation(this.getIntResultByString(resultObjet, "nombre"));
				perso.ajouterObjet(objet);
			}
			this.closeResultSet(resultObjet);
			perso.setStrategie(new StrategieImmobile(perso));
			if ("personnage".equals(type)) {
	...
	private Personnage loadPersonnagePartie (ResultSet result) {
		CreateCharactersFactoryAbstract factoryPersonnage = new FactoryPersonnageFireEmblem();
		Personnage personnage = (Personnage)factoryPersonnage.createCharacter(this.getStringResultByString(result, "nom"), null, TypePersonnageFireEmblem.getTypeByName(this.getStringResultByString(result, "classe")));
